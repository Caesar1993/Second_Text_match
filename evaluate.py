...
class Evaluator:
    ...
    def knwb_to_vector(self):
        ...
        self.question_ids = []
        #standard_question_index是schema的标准问序列，question_ids是标准问的多个问法语句
        for standard_question_index, question_ids in self.train_data.dataset.knwb.items():
            for question_id in question_ids:
                #记录问题编号到标准问题标号的映射，用来确认答案是否正确。
                #按照字典形式，key为问题编号，value为问题对应的标准问
                self.question_index_to_standard_question_index[len(self.question_ids)] = standard_question_index
                self.question_ids.append(question_id)#存储每一个问题的数字表示，按照列表形式
        with torch.no_grad():#不用构建图，不需要执行梯度反传的情况下使用
            question_matrixs = torch.stack(self.question_ids, dim=0)#将多个1878*20的tensor按照前后位置拼接起来，形成一个1878*20
            if torch.cuda.is_available():
                question_matrixs = question_matrixs.cuda()
            self.knwb_vectors = self.model(question_matrixs)#将知识库（训练数据）的question_matrixs的向量进行编码，形成1878*128的向量
            #将所有向量都作归一化 v / |v|
    ...
    def eval(self, epoch):
        ...
        for index, batch_data in enumerate(self.valid_data):
            ...
            with torch.no_grad():
                test_question_vectors = self.model(input_id) #将测试数据进行编码，不输入labels，使用模型当前参数进行预测
    ...
    def write_stats(self, test_question_vectors, labels):
        ...
        for test_question_vector, label in zip(test_question_vectors, labels):
            ...
            #将所有问题进行排序，得到最相似的问题，再求对应的标准问题即可
